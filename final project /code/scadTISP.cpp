...
inline arma::vec scadThresholding(arma::vec z, double lambda, double a){
  int n=z.n_elem;
  arma::vec x(n);
  for(int i=0;i<n;i++){
    if(-lambda<=z(i)&z(i)<=lambda)
      x(i)=0;
    else if(lambda<z(i)&z(i)<=2*lambda)
      x(i)=z(i)-lambda ;
    else if(-2*lambda<=z(i)&z(i)<-lambda)
      x(i)=z(i)+lambda;
    else if(2*lambda<z(i)&z(i)<=a*lambda)
      x(i)=((a-1)*z(i)-a*lambda)/(a-2);
    else if(-a*lambda<=z(i)&z(i)<-2*lambda)
      x(i)=((a-1)*z(i)+a*lambda)/(a-2);
    else
      x(i)=z(i);
  }
...
// [[Rcpp::export]]
arma::vec scadTISP(arma::mat X0, arma::vec y0,double lambda,double a){
  int n=X0.n_rows;
  int p=X0.n_cols;
  ...
  int k=0;
  do{
    beta=scadThresholding(A*beta0+c,lambda,a);
    t=beta0;
    beta0=beta;
    k++;
...
// [[Rcpp::export]]
double scadTISPcrossValidation(arma::mat X, arma::vec y, int K, arma::vec Lambda,double a){
  int N = X.n_rows;
  int size=N/K;
  int testnum=N-size;
  int M=Lambda.n_elem;
  ...
  for(int j=0;j<=M-1;j++ ){
    for(int i=1;i<=K;i++){
      ...
      betahat=scadTISP(Xtrain, ytrain,Lambda(j),a);
      yhat=Xtest*betahat;
