...
inline arma::vec hardThresholding(arma::vec z, double lambda){
  int n=z.n_elem;
  arma::vec x(n);
  for(int i=0;i<n;i++){
...
// [[Rcpp::export]]
arma::vec hardTISP(arma::mat X0, arma::vec y0,double lambda){
  int n=X0.n_rows;
  int p=X0.n_cols;
  ...
  int k=0;
  do{
    beta=hardThresholding(A*beta0+c,lambda);
    t=beta0;
    beta0=beta;
    k++;
...
// [[Rcpp::export]]
double hardTISPcrossValidation(arma::mat X, arma::vec y, int K, arma::vec Lambda){
  int N = X.n_rows;
  int size=N/K;
  int testnum=N-size;
  int M=Lambda.n_elem;
  ...
  for(int j=0;j<=M-1;j++ ){
    for(int i=1;i<=K;i++){
      ...
      betahat=hardTISP(Xtrain, ytrain,Lambda(j));
      yhat=Xtest*betahat;
